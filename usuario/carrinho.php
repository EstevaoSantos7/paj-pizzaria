...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['decrementar'])) {
        ...
        // Verifica se a quantidade é maior que 1 para permitir o decremento
        // Garantir que a quantidade nunca seja menor que 1
        $sql = "UPDATE carrinho SET quantidade = CASE 
                    WHEN quantidade > 1 THEN quantidade - 1 
                    ELSE quantidade 
                END WHERE usuario_id = ? AND produto_id = ?";
        if ($stmt = $conn->prepare($sql)) {
            $stmt->bind_param("ii", $usuario_id, $produto_id);
            $stmt->execute();
            $stmt->close();
